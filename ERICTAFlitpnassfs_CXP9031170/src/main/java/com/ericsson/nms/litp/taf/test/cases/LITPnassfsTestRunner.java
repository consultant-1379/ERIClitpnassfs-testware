...
public class LITPnassfsTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPnassfsTestRunner.class);
    ...
    @TestId(id = "CXP9031170-1", title = "Upgrade ERIClitpnassfs rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpnassfsRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("nassfs");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031170-2", title = "Run python test cases for ERIClitpnassfs package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpnassfsTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpnassfsTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031170-3", title = "Parse xml outputs from python tests")
